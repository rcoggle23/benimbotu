...
if(!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`**<a:uyar:1114935085406367794> Bu Komutu Kullanmak İçin __Yönetici__ Yetkisine Sahip Olmalısın.**`)
  ...
  if (args[0] == "aç") {
    ...
    message.reply(
      `<a:evet:1114956255480524910> Bot otorol \`${rol.name}\` olarak ayarlandı. Kapatmak için : \`${prefix}bototorol kapat\``
    );
  }

  if (args[0] == "kapat") {
    db.delete(`bototorol_${message.guild.id}`);
    message.channel.send("<a:evet:1114956255480524910> Sistem Sıfırlandı");
  }
};
