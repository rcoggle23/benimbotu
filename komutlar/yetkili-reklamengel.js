...
if (!frenzy) return message.reply(`<a:uyar:1114935085406367794> Hatalı Kullanım\nÖrnek Kullanım : \`.reklamengel aç #kanal\``)
  ...
  if(açıkkapalı) return message.reply(`<a:evet:1114956255480524910> Zaten reklam engel bu kanalda/belirttiğiniz kanalda aktif!`)
    
db.set(`reklamEngelFrenzy_${code.id}`,'açık')
message.reply(`<a:evet:1114956255480524910> Reklam engel sistemi başarıyla bu kanalda/belirttiğiniz kanalda aktif edildi!`)
  }
  
  if (frenzy == 'kapat') {
  let açıkkapalı = await db.fetch(`reklamEngelFrenzy_${code.id}`)
  if(!açıkkapalı) return message.reply(`<a:evet:1114956255480524910> Zaten Reklam engel bu kanalda/belirttiğiniz kanalda deaktif!`)
    
db.delete(`reklamEngelFrenzy_${code.id}`)
message.reply(`<a:evet:1114956255480524910> Reklam engel sistemi başarıyla bu kanalda/belirttiğiniz kanalda deaktif edildi!`)
}
};
exports.conf = {
  enabled: true,
